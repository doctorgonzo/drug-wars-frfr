...
public class ContractManager : MonoBehaviour
{
    ...
    public void OnDealerRestocked(Dealer dealer)
    {
        ...
        // ONE CONTRACT PER CITY — if any other dealer in this city already has an offer
        // or active contract, skip rolling. The first dealer to restock claims the city's
        // contract slot until that contract resolves.
        var gsm = GameSessionManager.Instance;
        var city = gsm?.FindCityOfDealer(dealer);
        if (city != null && city.Dealers != null)
        {
            foreach (var other in city.Dealers)
            {
                if (other == null || other == dealer) continue;
                bool otherHasOffer = _offers.ContainsKey(other.GetInstanceID());
                bool otherHasActive = _active.Any(c => c.dealerName == other.Name && c.state == ContractState.Accepted);
                if (otherHasOffer || otherHasActive) { _offers.Remove(id); return; }
            }
        }
        ...
        // Dealer doesn't offer contracts for drugs they sell — gather drug names this
        // dealer carries, then build a pool of drugs from OTHER dealers' inventories.
        var sellsThese = new HashSet<string>();
        if (dealer.Inventory != null)
            foreach (var it in dealer.Inventory)
                if (it is Drug d) sellsThese.Add(d.Name);

        var pool = new List<Drug>();
        var seen = new HashSet<string>();
        if (gsm?.AllCities != null)
        {
            foreach (var c in gsm.AllCities)
            {
                if (c?.Dealers == null) continue;
                foreach (var dlr in c.Dealers)
                {
                    if (dlr?.Inventory == null) continue;
                    foreach (var it in dlr.Inventory)
                        if (it is Drug dr && !sellsThese.Contains(dr.Name) && seen.Add(dr.Name))
                            pool.Add(dr);
                }
            }
        }

        if (pool.Count == 0) { _offers.Remove(id); return; }

        var drug = pool[UnityEngine.Random.Range(0, pool.Count)];
